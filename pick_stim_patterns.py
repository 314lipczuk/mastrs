...
def _():
    ...
    def compose_pattern(segments, total_budget=BUDGET_MS,
                        first=FIRST_FRAME_STIM, last=LAST_FRAME_STIM):
        ...
        for i, seg in enumerate(segments):
            ...
            expo = shape_exposures(seg["coeffs"], len(frames),
                                   budget=total_budget * fracs[i])
            all_frames.append(frames)
            all_expo.append(expo)
        return np.concatenate(all_frames), np.concatenate(all_expo)

    def superpose_patterns(layers, total_budget=BUDGET_MS,
                           first=FIRST_FRAME_STIM, last=LAST_FRAME_STIM):
...
def _(np, pulse_positions, qmc, shape_exposures):
    ...
    for _c1, _c2, _c3, _pif in _scaled:
        ...
        patterns.append(
            {"c1": _c1, "c2": _c2, "c3": _c3, "pi": _pi,
             "frames": _frames, "expo": _expo}
        )
    return (patterns,)
...
def _(FIRST_FRAME_STIM, LAST_FRAME_STIM, np, patterns, plt):
    ...
    _fig.suptitle(f"{len(patterns)} stimulation patterns (Latin Hypercube)", fontsize=10)
    _fig.tight_layout()
    plt.gca()
    return
...
def _(mo):
    ...
    budget_split = mo.ui.slider(0.1, 0.9, 0.05, label="Seg 1 budget fraction", value=0.5)
    mo.hstack([
...
def _(
    ...
    _segments = [
        {"coeffs": np.array([seg1_c1.value, seg1_c2.value, seg1_c3.value]),
         "pi": int(round(seg1_pi.value)), "budget_frac": budget_split.value},
        {"coeffs": np.array([seg2_c1.value, seg2_c2.value, seg2_c3.value]),
         "pi": int(round(seg2_pi.value)), "budget_frac": 1 - budget_split.value},
    ]
...
def _(
    ...
    for _c1, _c2, _c3, _pif in _scaled_s:
        ...
        mixed_patterns.append(
            {"kind": "single", "frames": _frames, "expo": _expo,
             "label": f"[{_c1:+.1f},{_c2:+.1f},{_c3:+.1f}] pi{_pi}"}
        )

    _bounds_comp = np.array([
        [-3, 3], [-3, 3], [-3, 3], [1, 8],   # seg 1
        [-3, 3], [-3, 3], [-3, 3], [1, 8],   # seg 2
    ...
    for _row in _scaled_c:
        _segs = [
            {"coeffs": np.array(_row[0:3]),
             "pi": int(round(_row[3])), "budget_frac": _row[8]},
            {"coeffs": np.array(_row[4:7]),
             "pi": int(round(_row[7])), "budget_frac": 1 - _row[8]},
        ]
        _frames, _expo = compose_pattern(_segs, total_budget=BUDGET_MS)
        mixed_patterns.append(
            {"kind": "composed", "frames": _frames, "expo": _expo,
             "label": f"comp {_row[8]:.0%}"}
        )

    _bounds_sup = np.array([
        [-3, 3], [-3, 3], [-3, 3], [1, 8],   # layer A
        [-3, 3], [-3, 3], [-3, 3], [1, 8],   # layer B
    ], dtype=float)
    ...
    for _row in _scaled_sup:
        ...
        mixed_patterns.append(
            {"kind": "superposed", "frames": _frames, "expo": _expo,
...
@app.cell
def _(mo):
    export_normal_btn = mo.ui.run_button(label="Export normal-sampled patterns to CSV (Warning: floats; not sure if i should have quantized it somehow)")
    export_normal_btn
...
def _(
    ...
    _ex_a = shape_exposures(np.array([sup_a_c1.value, sup_a_c2.value, sup_a_c3.value]),
                            len(_fr_a), budget=_half)
    _fr_b = pulse_positions(int(round(sup_b_pi.value)))
    _ex_b = shape_exposures(np.array([sup_b_c1.value, sup_b_c2.value, sup_b_c3.value]),
                            len(_fr_b), budget=_half)
