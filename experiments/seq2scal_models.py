...
from experiments.seq2seq_data import load as load_dataset
...
def prepare_data(
    ...
    *,
    ...
    Split is the project-standard 70/10/20 (``test_size=0.2`` then ``0.125``)
    with ``random_state`` fixed so test windows are stable across runs.
    """
    cnr_all, stim_all, _ = load_dataset(data_source)
    n = len(cnr_all)
    ...
    return PreparedData(_pick(tr), _pick(va), _pick(te), n, data_source)
