...
def _(mo, parse_bool):
    ...
    EXPERIMENT_NAME = mo.cli_args().get("name", "lstm_seq2scal_mdn_minfeats")
...
def _(DRY_RUN, EXPERIMENT_NAME, IS_HEADLESS, MODE, form, mo, resolve_configs):
    # encoder gets CNR + fluence + baseline (3 channels); decoder gets fluence (1)
    _cfgs, data_source, _ctx_display = resolve_configs(
        ...
        always={"m": {"encoder_dim": 3, "stim_dim": 1, "data_source": "real"}},
        default_source="real",
...
def _(DRY_RUN, data_source, mo):
    # Load real data directly so we can extract per-cell scalars (median_cnr_0_9)
    # alongside the per-step fluence trajectory. We bypass seq2seq_data.load() here
    # because the encoder/decoder feature split is bespoke for this experiment.
    import pandas as _pd
    from notebooks.experiment.preprocessing import make_tracks

    if data_source != "real":
        raise ValueError(
            f"This minfeats variant only supports `real` (got {data_source!r})"
        )

    # dataset.parquet is the already-cleaned merged output of preprocessing.py
    # — it already has median_cnr_0_9 + stim features, so skip re-cleaning
    ...
    cnr_tr, fluence_tr, baseline_tr = (
        cnr_all[_tr_ids], fluence_all[_tr_ids], baseline_all[_tr_ids],
    )
    cnr_va, fluence_va, baseline_va = (
        cnr_all[_va_ids], fluence_all[_va_ids], baseline_all[_va_ids],
    )
    cnr_te, fluence_te, baseline_te = (
        cnr_all[_te_ids], fluence_all[_te_ids], baseline_all[_te_ids],
    )
...
def _(
    ...
    artifacts = run_experiment(
        ...
        dataset={
            "train": (cnr_tr, fluence_tr, baseline_tr),
            "val": (cnr_va, fluence_va, baseline_va),
        },
...
@app.cell
def _(baseline_te, cnr_te, fluence_te, mo, model_config_used, training_config):
    ...
    test_ds = Seq2SeqDataset(
        cnr_te_used, fluence_te_used, baseline_te_used, H, F_, stride=_test_stride
    )
