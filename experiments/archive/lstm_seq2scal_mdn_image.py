...
with app.setup:
    ...
    class ModelConfig(BaseModel):
        ...
        variant: str = "seq2scalar_mdn_ar_tf_image"
    ...
    class Seq2SeqImageDataset(Seq2SeqDataset):
        ...
        Each sample returns 4 tensors:
            enc_in     : (history_len, 1 + n_stim)
            dec_stim   : (future_len, n_stim)
            dec_target : (future_len,)
            images_h   : (history_len, 1, H_px, W_px)
        """

        def __init__(self, cnr, stim, images, history_len, future_len, stride=5):
            self.samples = []
            total = history_len + future_len
            for i in range(len(cnr)):
                cnr_i = cnr[i]
                ...
                while t + total <= T:
                    ...
                    self.samples.append(
                        (enc_in, dec_stim.T, dec_target, img_window)
                    )
                    t += stride
    ...
    class Seq2ScalarMDNImage(Seq2ScalarMDN):
        ...
        def fit(dataset, ctx):
            ...
            dataset: {"train": (cnr, stim, images), "val": (cnr, stim, images)}
                     where images is per-cell object array of (T_i, H_px, W_px),
                     or {"train": (cnr, stim), "val": (cnr, stim)} for the
                     no-image fallback (use_images=False).
            """
            mcfg = ctx.model_config
            tcfg = ctx.training_config
            ...
            _has_images = mcfg.use_images and len(dataset["train"]) == 3
            if _has_images:
                cnr_tr, stim_tr, images_tr = dataset["train"]
                cnr_va, stim_va, images_va = dataset["val"]
                train_ds = Seq2SeqImageDataset(
                    cnr_tr, stim_tr, images_tr,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
                val_ds = Seq2SeqImageDataset(
                    cnr_va, stim_va, images_va,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
                run_epoch_fn = _run_epoch_img
            else:
                cnr_tr, stim_tr = dataset["train"][:2]
                cnr_va, stim_va = dataset["val"][:2]
                train_ds = Seq2SeqDataset(
                    cnr_tr, stim_tr,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
...
def _(mo, parse_bool):
    ...
    EXPERIMENT_NAME = mo.cli_args().get("name", "lstm_seq2scal_mdn_image")
...
def _(
    ...
    _cfgs, data_source, _ctx_display = resolve_configs(
        ...
        always={"m": {
            "encoder_dim": 1 + n_stim + _enc_extra,
            "stim_dim": n_stim,
...
def _(DRY_RUN, data_source, mo):
    ...
    from notebooks.experiment.preprocessing import make_tracks, DEFAULT_STIM_COLS

    if data_source != "real":
        raise ValueError(
            f"This image variant only supports `real` (got {data_source!r})"
        )

    _df = _pd.read_parquet("dataset.parquet")

    cnr_all, stim_all, _meta = make_tracks(
        _df,
    ...
    cnr_tr, stim_tr = cnr_all[tr_ids], stim_all[tr_ids]
    cnr_va, stim_va = cnr_all[va_ids], stim_all[va_ids]
    cnr_te, stim_te = cnr_all[te_ids], stim_all[te_ids]
...
def _(
    ...
    if USE_IMAGES:
        _train_tuple = (cnr_tr, stim_tr, images_tr)
        _val_tuple = (cnr_va, stim_va, images_va)
    else:
        _train_tuple = (cnr_tr, stim_tr)
        _val_tuple = (cnr_va, stim_va)
...
def _(
    ...
    if USE_IMAGES and images_te_used is not None:
        test_ds = Seq2SeqImageDataset(
            cnr_te_used, stim_te_used, images_te_used, H, F_, stride=_test_stride
        )
    else:
        test_ds = Seq2SeqDataset(
            cnr_te_used, stim_te_used, H, F_, stride=_test_stride
        )
    ...
    return F_, H, cnr_te_used, images_te_used, stim_te_used, test_ds
...
def _(
    ...
    with torch.no_grad():
        for _i in range(F_):
            ...
            if _use_img:
                _next_in = torch.cat(
                    [_next_abs, _stim_batch[:, _i, :], _last_img_emb.squeeze(1)],
                    dim=-1,
                ).unsqueeze(1)
            else:
                _next_in = torch.cat(
...
def _(
    ...
    with torch.no_grad():
        for _i in range(F_):
            ...
            if _use_img_k:
                _next_in = torch.cat(
                    [_next_abs, _stim_b[:, _i, :], _last_img_emb_k.squeeze(1)],
                    dim=-1,
                ).unsqueeze(1)
            else:
                _next_in = torch.cat(
                    [_next_abs, _stim_b[:, _i, :]], dim=-1
                ).unsqueeze(1)
