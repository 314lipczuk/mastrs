...
def evaluate(model, test_arrays, mcfg, *, device, test_stride=10, fluence_on=None):
    ...
    by_window_class = {}
    for cls, msk in (("interior", ~onset_mask), ("onset", onset_mask)):
        if msk.any():
            by_window_class[cls] = {
                "n": int(msk.sum()),
    ...
    full = {
        ...
        "absolute": {
            "test_abs_mae": float(abs_cnr_resid.mean()),
            "test_abs_rmse": float(np.sqrt((abs_cnr_resid ** 2).mean())),
            "q4_abs_mae": float(abs_cnr_resid[q4].mean()) if q4.any() else float("nan"),
            "q4_abs_rmse": (
                float(np.sqrt((abs_cnr_resid[q4] ** 2).mean())) if q4.any() else float("nan")
            ),
            "mean_abs_sigma": float(abs_sigma.mean()),
            "mean_1sigma_coverage": float(np.mean(abs_cov_1sig)),
        },
        ...
        "by_window_class": by_window_class,
        "baseline_prepend": prepend,
    ...
    per_step = {
        ...
        "abs_mae": [float(abs_cnr_resid[:, s].mean()) for s in range(mcfg.future_len)],
        "abs_rmse": [
            float(np.sqrt((abs_cnr_resid[:, s] ** 2).mean())) for s in range(mcfg.future_len)
        ],
        "abs_coverage_1sigma": abs_cov_1sig,
    }
