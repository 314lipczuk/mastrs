...
def _(DRY_RUN, data_source, mo, model_config):
    ...
    prep = prepare_data(
        data_source, ewma_slow_alpha=_slow, ewma_fast_alpha=_fast, dry_run=DRY_RUN
    )
    mo.md(
...
@app.cell
def _(device, ev, mo, model, model_config_used, test_arrays):
    ...
    mo.md(f"## Evaluation\n\n```\n{summary}\n```")
    return eval_result, step0_diag, summary
