...
with app.setup:
    ...
    class Seq2ScalarMDN(nn.Module):
        ...
        def fit(dataset, ctx):
            ...
            pretrained_sd = dataset.pop("pretrained_state_dict", None)
...
def _(mo, parse_bool):
    ...
    EXPERIMENT_NAME = mo.cli_args().get("name", "lstm_seq2scal_mdn_strat_cosine_pretrained")
...
def _(IS_HEADLESS, MODE, mo, repo_root, results_read_sources):
    if MODE == "train" and not IS_HEADLESS:
        ...
        pretrained_source_selector = mo.ui.dropdown(
            options=list(_sources.keys()), value="Local", label="Pretrained source",
        )
    else:
        pretrained_source_selector = None
    pretrained_source_selector if pretrained_source_selector is not None else mo.md("")
    return (pretrained_source_selector,)
...
def _(
    ...
    MODE,
    mo,
    pretrained_source_selector,
    repo_root,
    results_read_sources,
    scan_experiment_dirs,
):
    if MODE == "train" and not IS_HEADLESS and pretrained_source_selector is not None:
        _src_root = Path(results_read_sources(repo_root)[pretrained_source_selector.value])
        _choices = scan_experiment_dirs(_src_root)
        if _choices:
            pretrained_picker = mo.ui.dropdown(
                options=["(none)"] + _choices, value="(none)", label="Pretrained experiment (synthetic-trained)",
            )
            pretrained_load_button = mo.ui.button(
                value=0, on_click=lambda n: n + 1, label="Load pretrained",
            )
            pretrained_source_root = _src_root
            _ui = mo.vstack([pretrained_picker, pretrained_load_button])
        else:
            pretrained_picker = None
            pretrained_load_button = None
            pretrained_source_root = None
            _ui = mo.md(f"No experiments under `{_src_root}`.")
    else:
        pretrained_picker = None
        pretrained_load_button = None
        pretrained_source_root = None
        _ui = mo.md("")
    _ui
    return pretrained_load_button, pretrained_picker, pretrained_source_root
...
def _(
    ...
    load_experiment,
    mo,
    pretrained_load_button,
    pretrained_picker,
    pretrained_source_root,
):
    _clicked = pretrained_load_button is not None and pretrained_load_button.value > 0
    if MODE == "train" and IS_HEADLESS:
        _path = mo.cli_args().get("pretrained_path", None)
        if _path:
            pretrained_state_dict = load_experiment(_path).reconstruct_model().state_dict()
        else:
            pretrained_state_dict = None
    elif MODE == "train" and _clicked and pretrained_picker is not None and pretrained_picker.value != "(none)":
        pretrained_state_dict = load_experiment(
            str(pretrained_source_root / pretrained_picker.value)
        ).reconstruct_model().state_dict()
    else:
        pretrained_state_dict = None
    mo.md(f"Pretrained weights: **{'loaded ✓' if pretrained_state_dict is not None else 'none — training from scratch'}**")
...
def _(
    ...
    _dataset = {"train": (cnr_tr, stim_tr), "val": (cnr_va, stim_va)}
    if pretrained_state_dict is not None:
        _dataset["pretrained_state_dict"] = pretrained_state_dict
