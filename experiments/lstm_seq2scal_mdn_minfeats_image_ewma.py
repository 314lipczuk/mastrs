...
with app.setup:
    ...
    class ModelConfig(BaseModel):
        ...
        variant: str = "seq2scalar_mdn_ar_tf_minfeats_image_ewma"
    ...
    def compute_ewma(x, alpha):
        """Causal EWMA: ewma[0] = x[0]; ewma[t] = α·x[t] + (1-α)·ewma[t-1]."""
    ...
    class Seq2SeqDataset(Dataset):
        """Encoder sees CNR + per-step fluence + per-cell baseline + EWMA(slow/fast) of CNR.
        ...
        Inputs are per-cell object arrays:
            ...
            ewma_slow : (n_cells,) of (T_i,) EWMA of CNR (small alpha)
            ewma_fast : (n_cells,) of (T_i,) EWMA of CNR (large alpha)
        """
        ...
        def __init__(
            ...
            for i in range(len(cnr)):
                ...
                ewma_s_i = np.asarray(ewma_slow[i], dtype=np.float32)
                ewma_f_i = np.asarray(ewma_fast[i], dtype=np.float32)
                T = len(cnr_i)
                t = 0
                while t + total <= T:
                    ...
                    enc_ewma_s = ewma_s_i[t : t + history_len]
                    enc_ewma_f = ewma_f_i[t : t + history_len]
                    ...
                    enc_in = np.stack(
                        [enc_cnr, enc_flu, enc_base, enc_ewma_s, enc_ewma_f],
                        axis=-1,
                    )  # (history_len, 5)
    ...
    class Seq2ScalarMDN(nn.Module):
        ...
        def forward(self, encoder_input, future_stim, targets=None, tf_ratio=0.0):
            ...
            for i in range(F):
                ...
                if i < F - 1:
                    ...
                    # encoder window: [cnr, fluence, baseline, ewma_slow, ewma_fast];
                    # baseline is constant per cell; ewma is recomputed from
                    # sampled next_cnr.
                    last_baseline = current_window[:, -1, 2:3]
                    last_ewma_s = current_window[:, -1, 3:4]
                    last_ewma_f = current_window[:, -1, 4:5]
                    use_teacher = (
                    ...
                    a_s = self.cfg.ewma_slow_alpha
                    a_f = self.cfg.ewma_fast_alpha
                    next_ewma_s = a_s * next_cnr_abs + (1.0 - a_s) * last_ewma_s
                    next_ewma_f = a_f * next_cnr_abs + (1.0 - a_f) * last_ewma_f
        ...
        def fit(dataset, ctx):
            ...
            dataset: {"train": (cnr, fluence, baseline, ewma_slow, ewma_fast), "val": (...)}
                     where cnr/fluence/ewma_* are per-cell object arrays of (T_i,)
            ...
            cnr_tr, flu_tr, base_tr, ews_tr, ewf_tr = dataset["train"]
            cnr_va, flu_va, base_va, ews_va, ewf_va = dataset["val"]
    ...
    class Seq2SeqImageDataset(Seq2SeqDataset):
        ...
        Each sample returns 4 tensors:
            enc_in     : (history_len, 5)  [cnr, fluence, baseline, ewma_slow, ewma_fast]
    ...
    class Seq2ScalarMDNImage(Seq2ScalarMDN):
        ...
        def forward(
            ...
            for i in range(F):
                ...
                if i < F - 1:
                    ...
                    #   [cnr, fluence, baseline, ewma_slow, ewma_fast, image_embed]
                    last_baseline = current_window[:, -1, 2:3]
                    last_ewma_s = current_window[:, -1, 3:4]
                    last_ewma_f = current_window[:, -1, 4:5]
                    use_teacher = (
        ...
        def fit(dataset, ctx):
            ...
            Encoder window features are [cnr, fluence, baseline, ewma_slow, ewma_fast]
            (+ image_embed if use_images=True). Decoder receives fluence only.

            dataset:
              with images:    {"train": (cnr, fluence, baseline, ewma_slow, ewma_fast, images),
                               "val":   (cnr, fluence, baseline, ewma_slow, ewma_fast, images)}
              without images: {"train": (cnr, fluence, baseline, ewma_slow, ewma_fast),
                               "val":   (cnr, fluence, baseline, ewma_slow, ewma_fast)}
            """
            mcfg = ctx.model_config
            tcfg = ctx.training_config
            ...
            _has_images = mcfg.use_images and len(dataset["train"]) == 6
            if _has_images:
                cnr_tr, flu_tr, base_tr, ews_tr, ewf_tr, images_tr = dataset["train"]
                cnr_va, flu_va, base_va, ews_va, ewf_va, images_va = dataset["val"]
                train_ds = Seq2SeqImageDataset(
                    cnr_tr, flu_tr, base_tr, ews_tr, ewf_tr, images_tr,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
                val_ds = Seq2SeqImageDataset(
                    cnr_va, flu_va, base_va, ews_va, ewf_va, images_va,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
                run_epoch_fn = _run_epoch_img
            else:
                cnr_tr, flu_tr, base_tr, ews_tr, ewf_tr = dataset["train"][:5]
                cnr_va, flu_va, base_va, ews_va, ewf_va = dataset["val"][:5]
                train_ds = Seq2SeqDataset(
                    cnr_tr, flu_tr, base_tr, ews_tr, ewf_tr,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
                val_ds = Seq2SeqDataset(
                    cnr_va, flu_va, base_va, ews_va, ewf_va,
                    mcfg.history_len, mcfg.future_len,
                    stride=tcfg.train_stride,
                )
...
def _():
    ...
    from experiments.seq2seq_data import AVAILABLE_DATASETS, REAL_DATASET_PATHS
...
def _(mo, parse_bool):
    ...
    EXPERIMENT_NAME = mo.cli_args().get("name", "lstm_seq2scal_mdn_minfeats_image_ewma")
...
def _(
    ...
    # encoder gets [cnr, fluence, baseline, ewma_slow, ewma_fast] (5) +
    # image_embed (E if use_images). decoder gets fluence (n_stim = 1)
...
@app.cell
def _(DRY_RUN, REAL_DATASET_PATHS, data_source, mo, model_config):
    ...
    if data_source not in REAL_DATASET_PATHS:
        raise ValueError(
            f"This minfeats+image variant only supports real-data sources "
            f"({list(REAL_DATASET_PATHS)!r}); got {data_source!r}"
        )
    _parquet_path = REAL_DATASET_PATHS[data_source]

    _df = _pd.read_parquet(_parquet_path)
    ...
    # Per-cell EWMA of CNR (slow + fast). Recomputed at AR rollout time
    # inside the model using the same alphas; here we just provide the
    # training-time values.
    _a_s = model_config.ewma_slow_alpha
    _a_f = model_config.ewma_fast_alpha
    ewma_slow_all = np.empty(len(cnr_all), dtype=object)
    ewma_fast_all = np.empty(len(cnr_all), dtype=object)
    for _i in range(len(cnr_all)):
        _c = np.asarray(cnr_all[_i], dtype=np.float32)
        ewma_slow_all[_i] = compute_ewma(_c, _a_s)
        ewma_fast_all[_i] = compute_ewma(_c, _a_f)
    ...
    ewma_slow_tr, ewma_fast_tr = ewma_slow_all[tr_ids], ewma_fast_all[tr_ids]
    ewma_slow_va, ewma_fast_va = ewma_slow_all[va_ids], ewma_fast_all[va_ids]
    ewma_slow_te, ewma_fast_te = ewma_slow_all[te_ids], ewma_fast_all[te_ids]
    ...
    **Data:** {n_traj} trajectories x {_len_str} timepoints (`{data_source}`)
    ...
    Encoder features: CNR + fluence_mJ_cm2 + median_cnr_0_9 (baseline)
    + EWMA_slow(α={_a_s:.3f}) + EWMA_fast(α={_a_f:.3f}) of CNR. Decoder: fluence_mJ_cm2.
    """)
    return (
        baseline_te,
        baseline_tr,
...
def _(
    ...
    if USE_IMAGES:
        _train_tuple = (
            cnr_tr, fluence_tr, baseline_tr, ewma_slow_tr, ewma_fast_tr, images_tr,
        )
        _val_tuple = (
            cnr_va, fluence_va, baseline_va, ewma_slow_va, ewma_fast_va, images_va,
        )
    else:
        _train_tuple = (
            cnr_tr, fluence_tr, baseline_tr, ewma_slow_tr, ewma_fast_tr,
        )
        _val_tuple = (
            cnr_va, fluence_va, baseline_va, ewma_slow_va, ewma_fast_va,
        )
...
def _(
    ...
    ewma_slow_te_used = ewma_slow_te
    ewma_fast_te_used = ewma_fast_te
    ...
    if USE_IMAGES and images_te_used is not None:
        test_ds = Seq2SeqImageDataset(
            ...
            ewma_slow_te_used, ewma_fast_te_used, images_te_used,
            H, F_, stride=_test_stride,
        )
    else:
        test_ds = Seq2SeqDataset(
            cnr_te_used, fluence_te_used, baseline_te_used,
            ewma_slow_te_used, ewma_fast_te_used,
            H, F_, stride=_test_stride,
        )
...
def _(device, mo, model, n_stim, pl, test_ds):
    ...
    def _zero_enc_stim(b):
        # only zero the fluence channel; leave baseline + EWMA features intact
        b[..., 1:1 + n_stim] = 0.0
